...
export default async req=>{
 ...
 try{
  ...
  if(!/^\d{6}$/.test(pair)||id.length<12)return Response.json({error:"绑定二维码无效"},{status:400});
  ...
  if(!rows[0])return Response.json({error:"二维码已使用或已过期，请在电脑版重新生成"},{status:409});
  return Response.json({ok:true});
 }catch(e){return Response.json({error:e.message||"绑定失败"},{status:500})}
};
